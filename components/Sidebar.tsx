  ...
  return (
    <>
      <div className={`flex items-center border-b border-blue-900 ${collapsed ? 'justify-center px-3 py-4' : 'gap-3 px-5 py-5'}`}>
        ...
        {!collapsed && (
          <div>
            <span className="block text-white text-xl font-black tracking-tight">CortexAI</span>
            <span className="text-slate-300 text-xs uppercase tracking-[0.3em]">Painel</span>
          </div>
        )}
      </div>
        ...
        {NAV_ITEMS.map(({ label, href, icon: Icon }) => {
          ...
          return (
            <Link
              ...
              className={`group flex items-center gap-3 rounded-3xl text-sm font-semibold transition-all duration-200 ease-out ${collapsed ? 'justify-center px-2 py-3' : 'px-4 py-3'} ${isActive ? 'bg-blue-500 text-white shadow-[0_20px_50px_rgba(15,23,42,0.35)]' : 'text-slate-200 hover:bg-blue-500/20 hover:text-white'} transform ${isActive ? '' : 'hover:-translate-x-0.5'}`}
            >
              <div className={`flex h-10 w-10 items-center justify-center rounded-3xl ${isActive ? 'bg-white/10 text-white' : 'bg-white/5 text-slate-200'} transition-all duration-200 group-hover:bg-blue-500/25`}>
      ...
      <div className={`border-t border-blue-900 ${collapsed ? 'pb-3 pt-2 space-y-2' : 'px-3 pb-4 pt-3 space-y-3'}`}>
        <div className={`${collapsed ? 'flex justify-center p-3' : 'flex items-center gap-3 px-4 py-3'} rounded-3xl ${espOnline ? 'bg-emerald-500/10 text-emerald-200' : 'bg-white/5 text-slate-300'} transition-colors duration-200`}>
        ...
        <button
          onClick={toggleTheme}
          className={`w-full flex items-center gap-3 rounded-3xl ${collapsed ? 'justify-center p-3' : 'px-4 py-3'} bg-white/5 text-slate-200 transition duration-200 hover:bg-white/10`}
        >
    ...
    <>
      ...
      {sidebarOpen && (
        <>
          ...
          <aside className="fixed top-0 left-0 z-50 flex h-screen w-[280px] flex-col bg-[#071431] border-r border-blue-900/60 shadow-2xl md:hidden">
      ...
      <aside className={`hidden md:flex md:w-[280px] md:flex-col md:bg-[#071431] md:border-r md:border-blue-900/60 md:shadow-2xl md:transition-all md:duration-300 ${collapsed ? 'md:w-[88px]' : ''}`}>
